...
| 实时快照、五档 | 无有效字段 | 使用独立 TDX socket 行情源 |
| 批量现价 | 空结果 | 使用独立 TDX socket 行情源 |
| K 线 | 只有代码和总数框架，无行数据 | 使用独立 TDX socket 行情源 |
| 除权除息 | 空结果 | 使用独立 TDX XDXR |
| 涨跌停状态 | 空结果 | 由五档推当前封板；历史封板时刻需其他来源 |
| 标的静态信息 | 无有效字段 | 公共静态源可补部分字段 |
| 券商融资标的资格 | 不可用 | 保持未知并交给券商柜台裁决 |

`QuantTradeDemo/backend/gateway/cn/sources/_tdx_proto.py` 已有零第三方依赖的公网 TDX socket
实现。2026-09-03 实测可连接 7709 行情服务器，并取得实时报价、最近 K 线和 XDXR 数据。
因此推荐保持交易与行情解耦：`cn-broker-api` 负责 headless 交易，量化项目现有 TDX source
负责行情，不继续为行情恢复完整 `Tdxw.exe`。
