...
放在 HTTP 层之外，因为**看门狗也要用同一把**：不共用的话会出现「看门狗正在拉起客户端，
同时 `/v1/session/ensure` 也去拉一遍」⇒ 两个客户端进程。让看门狗反过来 import HTTP 层，
依赖方向就错了。

REST 天然诱发重复调用——调用方库重试、超时后重发、监控周期打、浏览器刷新。
所以第二个请求**不排队**，直接拿到正在跑的那个任务号。
"""
