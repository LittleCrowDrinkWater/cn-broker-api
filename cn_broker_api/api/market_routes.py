...
⚠️ 这里给的是**客户端自己那条行情**（报单会被它的状态挡住），不是公网行情源的替代品。
...
def register(app: Flask, ctx: ApiContext) -> None:
    ...
    def klines():  # noqa: ANN202
        ...
        夜间回补。
        ⚠️ 分钟线**不刷缓存基本就是空的** ⇒ 要分钟线就带 `refresh=true`（慢几秒）。
        ⚠️ 单位随响应的 `units` 一起给：K 线的成交量是**股**、成交额是**万元**，
    ...
    def limit_status():  # noqa: ANN202
        ...
        ⚠️ 名字是「状态」而不是「涨跌停价」——**它不给价格**，价格仍要按板别与昨收自己算。
        ⚠️ 字段是厂商的原始键名，语义厂商没写、本服务不猜（猜错一个键就会把「没封板」
        读成「封住了」）。
        """
        codes = _codes()
    ...
    def dividends(code: str):  # noqa: ANN202
        ...
        ⚠️ 四个数值字段照抄厂商的列序，**单位没实测过** ⇒ 拿它算复权因子之前先与已有来源
