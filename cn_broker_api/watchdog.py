"""看门狗：盯着客户端进程在不在，掉了就拉起来。它只保证客户端活着，本服务自己活着
由计划任务保证。

刻意不做的三件事：**绝不填密码**（额度按天算而它每分钟醒一次，第一次失败就在没人看着时
把当天额度烧光 ⇒ 它保证进程活着，不是通道可用）；**绝不 kill**（"没响应"和"没进程"是两件
事，杀掉重启会打断正在飞的委托）；**一次只拉一个**（同时拉两个会让客户端还在初始化就又弹
一个窗口，而登录那侧第一步是先分类是哪道门）。

与 `/v1/session/ensure` 共用同一把单飞锁（不共用就是两个客户端进程），但只在真要起进程时
才抢：枚举进程零成本，为它抢锁会让心跳把 ensure 挡在门外。时段闸刻意不判节假日——引一份
交易日历进来等于给这个只该说 HTTP 的服务加一个数据依赖。
"""
...
class Watchdog:
    ...
    def _act(self, now: datetime) -> Dict[str, Any]:
        ...
        if self.flight is not None:
            ...
            if not mine:
                # 已经有一趟 ensure 在跑，它自己会把进程拉起来 ⇒ 让开（额度已先记掉）。
                return {"action": "none",
