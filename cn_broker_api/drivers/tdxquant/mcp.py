"""自检那一侧的最小通道：MCP over HTTP，只回答「交易通道通不通」；真交易走 `client.py`。

`TPyth.dll` 自带一个 JSON-RPC 服务，端口属主就是客户端进程 ⇒ 没客户端就没服务端。
方法名与 `tqcenter` 的函数名一字不差。三条不能省的口径：

① 传输层失败与业务失败分开：连不上/非 2xx/协议错抛异常，`ErrorId != 0` 照常返回。混成一种，
   查询类就再也没法把「查不到」和「真的没有」分开（对账整表删持仓那一类错）。
② `urllib` 必须显式给空 `ProxyHandler`：本机系统代理在注册表里，`127.0.0.1` 靠 `<local>`
   例外躲过去只是侥幸；这台机器还有一层接管所有 TCP 的 TUN ⇒ 也不能拿 TCP 连通性当判据。
③ 拿到句柄不等于登录了交易：实测有 `ErrorId=0` 而 `Value:[]` 的形态 ⇒ 判据是资产里有资金
   字段，不是"调用没报错"。
"""
...
class McpClient:
    ...
    def __init__(self, mcp_url: str = "http://127.0.0.1:17709") -> None:
        ...
        #: 线索，吞掉就只剩一句我们自己的猜测。
        self.last_error: str = ""
        #: 空 ProxyHandler ＝ requests 那边 `trust_env=False` 的标准库版本（见 ②）。
