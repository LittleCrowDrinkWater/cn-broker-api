"""跟通达信客户端说话的最小通道：MCP over HTTP。

`TPyth.dll` 自带一个 JSON-RPC 服务，端口属主就是客户端进程（`Tdxw.exe`）——**没客户端就没
服务端**。方法名与 `tqcenter` 的函数名一字不差。

## 为什么是「最小」

母项目那份客户端有 585 行（双通道、连接身份、硬重置、`_reInitialized` 那个雷）。本服务在
搬家阶段只需要回答一件事：**交易通道通不通**。那就是「连一次 + 查一次资产 + 看有没有资金
字段」，三十行。等阶段二真要接管交易调用，再把完整的那份搬过来。

## 三条不能省的口径

① **传输层失败与业务失败必须分开。** 连不上/HTTP 非 2xx/协议错 ⇒ 抛异常；`ErrorId != 0`
   ⇒ 照常返回让调用方按各自语义判。混成一种，查询类就再也没法把「查不到」和「真的没有」
   分开——那是这个模块里代价最大的一类错（对账整表删持仓 / 从零重复建仓）。

② **`urllib` 必须显式给空 `ProxyHandler`。** 本机系统代理写在注册表里，
   `urllib.request.getproxies()` 在 Windows 上会读到它；`127.0.0.1` 靠 `<local>` 例外躲
   过去只是侥幸。2026-08-21 实测这台机器上还有一层接管**所有** TCP 的 TUN——连路由上不
   存在的保留地址都能"连上"。所以既不能信代理会放过 loopback，也不能拿 TCP 连通性当判据。

③ **拿到句柄不等于登录了交易。** 实测有 `ErrorId=0` 而 `Value:[]` 的形态（"成功但没内容"）。
   判据必须是**资产里有资金字段**，不是"调用没报错"。
"""
