...
⚠️ **只改了接缝，判据一个字符没动。** 尤其 `check_quote`：那段 2026-08-21 刚按实盘修过并做过
负向验证（抽掉时点那一维会红两条），retype 一遍等于把验证结果作废。

接缝三处：logger 换标准库；安装根目录从本服务的 config 来；`check_account` /
`check_trade_channel` 从吃 gateway 改成吃 `McpClient` + 账号。

原始说明——把「今天能不能真的报出单」拆成四个**可分辨**的问题。

动因（两条都是真事）：
  1. 「待用户确认」那道人工闸靠给客户端页面打补丁拆掉（设计 §7.11）。补丁失效的方向是安全的
     （回到人工点【发送】），**但盘中失效的代价不对称**——买入腿不成交只是少赚，14:57 还款腿
     不成交就是**负债过夜**。而补丁不在时唯一的现象是"没人点就没成交"，是最需要主动报警的
     那类静默失效。客户端自动升级会把 `webs/cfg/aireq.html` 冲掉。
  2. 客户端那一侧还有三个各自独立的前提，任何一个不成立都表现为"到点了什么都没发生"：
     客户端进程在不在（MCP 端口是它开的）、交易账号登录了没、客户端的行情侧通不通
     （报单会被「当前客户端行情处于断开状态」挡住，而**查询那时全是绿的**——2026-08 实测，
     四小时没人发现）。

⭐ 本模块的全部价值在于**把这四件事分开报**。它们的处置完全不同（去开客户端 / 去登录交易 /
去看行情连接 / 去重打补丁），混成一句"通道不可用"等于没说。这也是"先做能把『连不上』与
『连上了但没有』分开的实验"那条判据在代码里的落点。

🔴 **本模块只观测，不下单、不改任何文件**。它也**不阻断**调用方的后续动作：报单该发还是发
（信号会躺在队列里等人点，比不发好），自检的产出是一条 CRITICAL 事件。

⚠️ 每一项**能力边界**都写在各自的 docstring 里。尤其行情那项：它挡得住"行情侧完全没数"，
挡不住"缓存里有数而连接已断"——真正的判据只有第一笔委托的回执。所以它给的是 warn 不是保证。
"""
...
class Check:
    ...
    def line(self) -> str:
        mark = "✅" if (self.ok and not self.warn) else ("⚠️" if self.ok else "❌")
...
def check_quote(client, *, probe_symbol: str = DEFAULT_PROBE_SYMBOL,
    ...
    if live and bid1 <= 0 and ask1 <= 0:
        # 盘口两侧全空：涨跌停封板时**本来就**只有一侧，所以只在两侧都空时才说话，且只 warn
        # ——探针票是最活跃的那只，真封板的概率极低，但把它判成故障的代价是假警报。
        # ⭐ 盘前不进这一格：那时候盘口空是常态，warn 也是噪音。
        return Check("quote", "客户端行情", True,
